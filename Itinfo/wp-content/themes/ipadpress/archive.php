...
<div class="span-24" id="contentwrap">
	<div class="span-16">
 	  ...
 	  <?php /* If this is a category archive */ if (is_category()) { ?>
		<h2 class="pagetitle">Архив категории &#8216;<?php single_cat_title(); ?>&#8217; </h2>
 	  <?php /* If this is a tag archive */ } elseif( is_tag() ) { ?>
		<h2 class="pagetitle">Записи помеченные &#8216;<?php single_tag_title(); ?>&#8217;</h2>
 	  ...
 	  <?php /* If this is a yearly archive */ } elseif (is_year()) { ?>
		<h2 class="pagetitle">Архив <?php the_time('Y'); ?> года</h2>
	  <?php /* If this is an author archive */ } elseif (is_author()) { ?>
 	  ...
 	  <?php } ?>
		...
		<div <?php post_class() ?>>
				...
				<div class="postmeta"><img src="<?php bloginfo('template_url'); ?>/images/folder.png" /> Категория: <?php the_category(', ') ?> <?php if(get_the_tags('Метки:', ', ', '')) { ?> <img src="<?php bloginfo('template_url'); ?>/images/tag.png" /> <?php  the_tags('Метки: ', ', '); } ?>  <img src="<?php bloginfo('template_url'); ?>/images/comments.png" /> <?php comments_popup_link('Оставить комментарий!', 'Комментарии (1)', 'Комментарии (%)'); ?></div>
	...
	<?php else :
		...
		if ( is_category() ) { // If this is a category archive
			printf("<h2 class='pagetitle'>Извините, но в категории %s записей не нашлось.</h2>", single_cat_title('',false));
		} else if ( is_date() ) { // If this is a date archive
			echo("<h2 class='pagetitle'>Извините, но записей с указанной датой не существует.</h2>");
		} else if ( is_author() ) { // If this is a category archive
			$userdata = get_userdatabylogin(get_query_var('author_name'));
			printf("<h2 class='pagetitle'>Извините, но нет записей от автора %s.</h2>", $userdata->display_name);
		} else {
			echo("<h2 class='pagetitle'>Не найдено</h2>");
		}
